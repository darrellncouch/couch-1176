...
build() {
    echo "─────────────────────────────────────────────────────"
    echo "  Couch 1176 – FET Compressor Plugin"
    echo "─────────────────────────────────────────────────────"
    ...
    if ! pkg-config --exists x11 2>/dev/null && ! [ -f /usr/include/X11/Xlib.h ]; then
        echo "WARNING: X11 headers not found. Run: ./build.sh deps"
    fi
    ...
    echo "→ Building (this will download JUCE on first run ~200 MB)..."
    ...
    echo ""
    echo "  Standalone app:"
    find "$BUILD_DIR" -name "Couch 1176" -o -name "Couch_1176" 2>/dev/null | head -3 | \
        while read -r p; do echo "    $p"; done
    echo ""
